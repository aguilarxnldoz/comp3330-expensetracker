...
const updateExpenseSchema = z.object({
    ...
    fileUrl: z.string().min(1).max(500).nullable().optional(),
...
// translate incoming data into the database table format
...
// return the signed download URLs when an expense is read
...
export const expensesRoute = new Hono()
    ...
    .post("/", zValidator("json", createExpenseSchema), async (c) => {
        ...
        const createdWithUrl = created ? await withSignedDownloadUrl(created) : created;
    ...
    .delete("/:id{\\d+}", async (c) => {
        ...
        const deletedWithUrl = await withSignedDownloadUrl(deletedRow);
        return c.json({deleted: deletedWithUrl});
    });
